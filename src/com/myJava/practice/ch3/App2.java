...
public class App2 {
    public static void main(String[] args) {
        ...
        System.out.println("spring으로 시작하는 수업 목록");
        springClasses.stream()
                .filter(c -> c.getTitle().startsWith("spring"))
                .forEach(c -> System.out.print(c.getId() + " "));

        System.out.println("close 되지 않은 수업");
        springClasses.stream().filter(c -> !c.isClosed()).forEach(c -> System.out.print(c.getId() + " "));
        springClasses.stream().filter(Predicate.not(OnlineClass::isClosed)).forEach(c -> System.out.print(c.getId() + " "));
        ...
        List<List<OnlineClass>> hyejinEvents = new ArrayList<>();
        hyejinEvents.add(springClasses);
        hyejinEvents.add(javaClasses);


        System.out.println("두 수업 목록에 들어있는 모든 수업 아이디 출력");
        hyejinEvents.stream().flatMap(Collection::stream)
                        .forEach(c -> System.out.print(c.getId() + " "));
        System.out.println();
        ...
        System.out.println("스프링 수업 중에 제목이 spring이 들어간 제목만 모아서 List 만들기");
